...
<table cellpadding="0" cellspacing="0" class="margin_bottom_large">
    <tr>
        <td width="45"><a href="index.php"><img src="images/form_backup.gif" border="0" width="34" height="34"/></a>
        </td>
        <td class="title">
        ...
        <tr>
            ...
            <td>
                {foreach from=$views item=view}
                    ...
                    <label for="view_{$view.view_id}">{$view.view_name}</label>
                    <br/>
                {/foreach}
            </td>
        </tr>
        <tr>
            ...
            <td>
                {foreach from=$emails item=email}
                    <input type="checkbox" name="email_ids[]" value="{$email.email_id}" id="email_{$email.email_id}"
                           checked/>
                    <label for="email_{$email.email_id}">{$email.email_template_name}</label>
                    <br/>
                {/foreach}
                {if $emails|@count == 0}
